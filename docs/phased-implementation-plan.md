...
|---|---|---|---|
| Canonical domain lifecycle state machine | `in_progress` | `domains.lifecycle_state`, `domain_lifecycle_events`, `GET/POST /api/domains/[id]/lifecycle`, dashboard lifecycle controls, and acquisition/purchase automation hooks now enforce lifecycle progression (`score_candidate` -> `underwriting`, buy approvals -> `approved`, successful purchases -> `acquired`), plus downstream auto-progression now wired for `deploy` -> `build`, `campaign launch queue` -> `growth`, and positive `revenue` ledger entries -> `monetized`. Regression coverage now includes deploy processor + finance ledger lifecycle automation paths, and workflow dashboard now surfaces lifecycle automation telemetry (source counts + recent auto transitions). | Add lifecycle anomaly alerting (unexpected manual reversions/oscillation) and per-source automation SLO thresholds |
